...
import { AccommodationCategoryCard } from "@/components/AccommodationCategory/Card";
...
export const AccommodationCategoryListing = async ({ data, links }: Props) => {
  ...
  return (
    <ListView
      ...
      cards={data.map((item) => (
        <AccommodationCategoryCard key={item.id} slug={item.slug} />
      ))}
    />
  );
};
...
const fetchData = async (): Promise<z.infer<typeof ApiResponseSchema>> => {
  ...
  const response = await fetch(
    `${process.env.API_URL}/settings/page_accommodation_category_listing?${query}`,
    {
      next: {
        tags: ["global"],
      },
